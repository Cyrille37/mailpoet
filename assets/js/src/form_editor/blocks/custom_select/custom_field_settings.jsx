...
const CustomFieldSettings = ({
  ...
  const [localMultiple, setLocalMultiple] = useState(multiple);
  ...
  }), [localMandatory, localMultiple, localValues]);

  const hasUnsavedChanges = (localMandatory !== mandatory)
    || (localMultiple !== multiple)
  ...
  return (
    <div className="custom-field-settings" data-automation-id="custom_field_settings">
      ...
      />
      <ToggleControl
        label={MailPoet.I18n.t('blockMultiple')}
        checked={localMultiple}
        onChange={setLocalMultiple}
      />
      <SettingsPreview
        remove={remove}
        update={update}
